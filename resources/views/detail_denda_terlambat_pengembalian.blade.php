      ...
      <div class="card shadow" style="border-radius:50px !important;">
        ...
        <div class="card-body border-0">
          <div class="row">
            ...
            <div class="col-4">
              <h5 class="text-right">Kode Pembayaran <b>ALB-Fine-{{ $denda->id }}</b></h5>
            </div>
          </div>
                ...
                <div class="card-body">
                  <div class="row align-items-center">
                    <div class="col-12 align-items-center text-center">
                      @if($denda->tenant->foto != '')
                        <img src="{{ asset('storage/' . $denda->tenant->foto) }}" style="width:180px; height:180px;" alt="">
                      @else
                  ...
                  <div class="row">
                    <div class="col-6">
                      <div class="col mt-4">
                        ...
                        <h4 class="mt--2">{{ $denda->tenant->nama }}</h4>
                      </div>
                      <div class="col mt-3">
                        <h6 class="text-sm-start">Kontak:</h6>
                        <h4 class="mt--2">{{ $denda->tenant->no_hp }}</h4>
                      </div>
                      <div class="col mt-3">
                        <h6 class="text-sm-start">Kontak Darurat:</h6>
                        <h4 class="mt--2">{{ $denda->tenant->kontak_darurat }}</h4>
                      </div>
                    </div>
                    <div class="col-6">
                      <div class="col mt-3">
                        <h6 class="text-sm-start">Status:</h6>
                        @if($denda->konfirmasi_pembayaran === 0 and $denda->ket_konfirmasi == '')
                          <h3>Belum diverifikasi</h3>
                        @elseif($denda->konfirmasi_pembayaran === 1)
                          <h3>Pengajuan diterima</h3>
                        @elseif($denda->konfirmasi_pembayaran === 0 and $denda->ket_konfirmasi != '')
                      ...
                      <div class="col mt-3">
                        ...
                        <a href="{{ route('downloadBuktiPembayaran', ['id' => $denda->id]) }}">
                          <div class="contohgambar" style="position: relative;">
                            ...
                            <?php $buktiPembayaran = trim($denda->bukti_pembayaran, 'bukti_pembayaran/') ?>
          ...
          @foreach ($equipment as $equipment)
            <div class="card" style="padding:16px;">
              <div class="row">
                ...
                <?php
                  $tanggal_selesai = new DateTime($equipment->tanggal_selesai);
                  $tanggal_kembali = new DateTime($equipment->tanggal_kembali);
                  $terlambat = $tanggal_kembali->diff($tanggal_selesai);

                  $denda_hari = $terlambat->days * $equipment->harga_sewa_perhari;
                  $denda_jam = $terlambat->h * $equipment->harga_sewa_perjam;
                  $tanggal_orderan_mulai = new Carbon($equipment->tanggal_mulai);
                  $tanggal_orderan_selesai = new Carbon($equipment->tanggal_selesai);
                ...
                <div class="col-md-4 mt-5">
                  ...
                  @if($terlambat->days >= 1)
                    <div class="tes">
                      ...
                      <h4 class="mt--4">{{ $terlambat->days }} X {{ 'Rp. ' . number_format($equipment->harga_sewa_perhari, 2, ",", ".") }}</h4>
                    </div>
                  @elseif($terlambat->h >= 1)
                    <div class="tes">
                      <p>Jumlah jam sewa</p>
                      <h4 class="mt--4">{{ $terlambat->h }} X {{ 'Rp. ' . number_format($equipment->harga_sewa_perjam, 2, ",", ".") }}</h4>
                    </div>
                  @endif
                </div>
                <div class="col-md-4 mt-5 text-right" style="padding-right:30px">
                  <h4><b>Total</b></h4>
                  @if($terlambat->days >= 1)
                    <div class="tes">
                      <h4 class="mt-4">{{ 'Rp. ' . number_format($terlambat->days * $equipment->harga_sewa_perhari, 2, ",", ".") }}</h4>
                    </div>
                  @elseif($terlambat->h >= 1)
                    <div class="tes">
                      <h4 class="mt-4">{{ 'Rp. ' . number_format($terlambat->h * $equipment->harga_sewa_perjam, 2, ",", ".") }}</h4>
                    </div>
                  @endif
                </div>
            </div>
            ...
            <div class="row">
                ...
                <div class="col-6 text-right" style="padding-right:30px">
                  @if($terlambat->days >= 1)
                    <div class="tes">
                      <h3>{{ 'Rp. ' . number_format($terlambat->days * $equipment->harga_sewa_perhari, 2, ",", ".") }}</h3>
                    </div>
                  @elseif($terlambat->h >= 1)
                    <div class="tes">
                      <h3>{{ 'Rp. ' . number_format($terlambat->h * $equipment->harga_sewa_perjam, 2, ",", ".") }}</h3>
                    </div>
                  @endif
                </div>
            </div>
            @if($equipment->konfirmasi_denda == 0)
              <div class="row justify-content-end py-4">
                  <div style="padding-right:30px">
                    <form action="{{ route('bayarDenda', ['id' => $equipment->detail_id]) }}">
                        <input type="hidden" name="tanggal_keluar" id="tanggal_keluar" value="{{ Carbon::now() }}">
                        <button type="submit" name="verifikasi" class="btn btn-success"><span class="ni ni-check-bold"></span> Konfirmasi Pembayaran Denda</button>
                    </form>
                  </div>
              </div>
            @endif
        ...
        @if(Carbon::now() > $umurSkr->addDay(30))
            <?php $denda=0 ?>
        @elseif(Carbon::now() <= $umurSkr->addDay(30))
            <?php $denda= 0.2 * $total ?>
        @endif
        <div class="row">
          <div class="col-6">
        ...
        <div class="card-footer justify-content-end py-4">
          @if($denda->konfirmasi_pembayaran === 0 and $denda->ket_konfirmasi == '')
            <div class="col-12 text-right">
              <form action="{{ route('verifPembayaran', ['id' => $denda->id]) }}">
              ...
              <div class="modal fade" id="exampleModal" tabindex="-1" aria-labelledby="exampleModalLabel" aria-hidden="true">
                <div class="modal-dialog modal-dialog-centered">
                  <div class="modal-content">
                    ...
                    <form action="{{ route('tolakPembayaran', ['id' => $denda->id]) }}" method="POST" class="well" id="block-validate">
                      @csrf
                      @method('PUT')
