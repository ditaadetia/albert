          ...
          <div class="modal-dialog modal-dialog-centered">
            <div class="modal-content">
              <div class="modal-header">
                <h5 class="modal-title" id="exampleModalLabel">Rentang Waktu Rekapitulasi Pembayaran</h5>
                <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
              </div>
                <form action="{{ route('paymentExcel') }}" method="POST" class="well" id="block-validate">
                  @csrf
                  @method('PUT')
              ...
              @if($payments->count()>0)
                @foreach ($payments as $payment)
                  ...
                  <tr style="text-align:center !important;">
                    ...
                    <td>
                      {{ Carbon::parse($payment->created_at)->locale('id')->isoFormat('dddd, D MMMM YYYY') }}
                    </td>
                    {{-- <td>
